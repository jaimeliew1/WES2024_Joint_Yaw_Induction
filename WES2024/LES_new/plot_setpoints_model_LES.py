...
from WES2024.utils import FIGDIRFORPAPER
...
def plot_scatters(df, vars=["yaw", "Ctprime", "Cp"], name=""):
    ...
    plt.tight_layout()
    plt.savefig(FIGDIRFORPAPER / f"LES_model_comparison_{name}.png", dpi=300)
    plt.close()
...
def plot_Cp_sidebyside(df, var="Cp", name="", clim=[0.5, 1.05]):
    ...
    plt.savefig(FIGDIRFORPAPER / f"LES_model_turbineCp_{name}.png", dpi=300)
    plt.close()
...
def plot_setpoints_sidebyside(df, var="Cp", name="", clim=[0.5, 1.05], D=1.2, axs=None):
    ...
    df_x0 = df.filter(pl.col("turbine") == 20).select(
        ["simulator", pl.col("x").alias("x0"), pl.col("y").alias("y0")]
    )
    ...
    for x, y, ctp, yaw, yawdeg in sub.select(
        ["xnew", "ynew", "Ctprime", "yaw", "yawdeg"]
    ).iter_rows():
    ...
    plt.savefig(FIGDIRFORPAPER / f"LES_model_turbineCp_{name}.png", dpi=300, bbox_inches="tight")
    plt.close()
