"""
Script copied from a previous commit which calibrates to the no control
diamond layout power data. 
...
def retrieve_LES_output_files(
    ...
    if not overwrite:
        ...
        for file in files:
            ...
            if len(list(calibration_dir.glob(f"wdir{params['wdir']}*"))) == 0:
                to_keep.append(file)
        files = to_keep
...
class CalibrationCase:
    ...
    def run_model(self, a: float, b: float, c: float) -> WindfarmSolution:
        ...
        windfarm = Windfarm(rotor_model=ROTOR_MODEL, wake_model=wakemodel, TIamb=self.TIamb)
...
if __name__ == "__main__":
    ...
    for simulation in cases:
        ...
        with open(LES_input_dir / f"{simulation.casename}.json", "w") as f:
            print("Writing", simulation.casename)
